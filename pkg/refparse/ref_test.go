...
func TestReferrerParser(t *testing.T) {
	for _, data := range testData {
		t.Run(data.spec, func(t *testing.T) {
			res, err := Parse(data.uri)
			if err != nil {
				if data.known {
					t.Fatal("Expected to find data", err)
				}
				return
			}
			if !data.known {
			...
			if data.source != res.Referrer {
				t.Errorf("Bad referer  expected:%+v, got:%v", data.source, res.Referrer)
			}
			if data.medium != res.Medium {
				t.Errorf("Bad medium  expected:%+v, got:%v", data.medium, res.Medium)
			}
			if data.term != "" {
				if !hasTerm(res, data.term) {
					t.Errorf("Bad term for  expected:%+v, got:%v", data.term, res.Search)
				}
			}
		})
...
func hasTerm(r *Referrer, term string) bool {
